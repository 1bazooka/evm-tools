...
def found_token(pair, web3, config):
    ...
    embed = {"description": f"{config['explorer']}tx/{pair['transactionHash'].hex()}", "title": f"Transaction", "color": 15258703}
...
def listen_for_tokens(config):
    ...
    contract = web3.eth.contract(
        address=config["factory_address"], abi=config["listeningABI"]
    )
    ...
    try:
        ...
        loop.run_until_complete(
            asyncio.gather(token_loop(event_filter, 1, web3, config))
        )
...
def parse_args():
    ...
    run.add_argument(
        "--config", "-c", help="relative or full path to the config file", metavar="CONFIG", required=True
    )
